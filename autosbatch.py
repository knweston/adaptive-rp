...
class SlurmScript:
    ...
    def __init__(self, cluster, job_name, job_dir, out_dir, cmd, time="12:00:00", mem="8G", num_threads="2"):
...
def generate_jobs(apps, output, cluster, executable):
    ...
    if apps == "spec":
        requested_apps = ["410", "429", "450", "462", "470", "471", \
                          "473", "482", "600", "602", "603", "605", \
                          "607", "619", "620", "621", "623", "625", \
                          "627", "628", "631", "638", "641", "644", \
                          "649", "654", "657"]
        ...
        requested_apps = ["cassandra", "chirper", "drupal", "http", \
                          "kafka", "mediawiki", "tomcat", \
                          "verilator", "wordpress"]
    ...
    for idx, app in enumerate(requested_apps):
        cmd = f"{code_dir+'/run_champsim.sh'} {code_dir+'/bin/'+executable} 100 1000 {trace_dir+'/'+app+'.champsimtrace.xz'}"
        if apps == "cloud":
            cmd += f" -cloudsuite "
        
        if apps == "ripple":
            cmd = f"{code_dir+'/run_champsim.sh'} {code_dir+'/bin/'+executable} 0 100 {trace_dir+'/'+app+'.champsimtrace.gz'}"
