...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if ((cache_type == IS_LLC) && (way == LLC_WAY)) { // this is a bypass that does not fill the LLC
            ...
            // [Kevin]
            if ((smart_index) && (all_warmup_complete > NUM_CPUS) && !BASELINE_RUN) {
                smart_index->update(MSHR.entry[mshr_index].address, lower_level);
            }
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            // [Kevin]
            if ((smart_index) && (all_warmup_complete > NUM_CPUS) && !BASELINE_RUN) {
                smart_index->update(WQ.entry[index].address, lower_level);
            }
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
            ...
            if (way >= 0) { // read hit
                ...
                // [Kevin]
                if ((smart_index) && (all_warmup_complete > NUM_CPUS) && !BASELINE_RUN) {
                    smart_index->update(RQ.entry[index].address, lower_level);
                }
...
{
    if ((smart_index) && !BASELINE_RUN) {  // [Kevin]
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
{
    ...
    if ((smart_index) && !BASELINE_RUN) {  // [Kevin]
      uint32_t updated_set = smart_index->get_set(packet->address);
      if (updated_set != set) {
        cout << "ERROR: Filling set index mismatched" << endl;
        cout << "address:     " << packet->address << endl;   
        cout << "given set:   " << set << endl;
        cout << "updated set: " << updated_set << endl;
        cout << "map_ptr:     " << smart_index->get_map_ptr() << endl;
        cout << "last_sft:    " << smart_index->get_last_shift() << endl;
        cout << "curr_sft:    " << smart_index->get_curr_shift() << endl;
      }
    }
